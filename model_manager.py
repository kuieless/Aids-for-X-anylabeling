...
class ModelManager(QObject):
    ...
    def load_model_configs(self):
        ...
        try:
            ...
            for model in model_list:
                try:
                    ...
                    if config_file.startswith(":/"):  # Config file is in resources
                        ...
                        # 尝试多种编码方式
                        encodings = ['utf-8', 'gbk', 'iso-8859-1', 'latin1']
                        ...
                        if not model_config:
                            logger.error(f"Failed to read config file {config_file} with all encodings")
                            continue
            ...
            logger.error(f"Error in load_model_configs: {str(e)}")
            self.model_configs = []
            self.model_configs_changed.emit([])
    ...
    def load_custom_model(self, config_file):
        ...
        try:
            ...
            try:
                ...
                self.new_model_status.emit(
                    self.tr("Error in loading custom model: Cannot read config file.")
                )
                return
            ...
            try:
                ...
                self.new_model_status.emit(
                    self.tr("Error in saving custom model configuration.")
                )
                return
            ...
            try:
                ...
                logger.error(f"Error reloading model configs: {str(e)}")
                self.new_model_status.emit(
                    self.tr("Error in reloading model configurations.")
                )
                return
            ...
            try:
                ...
                self.new_model_status.emit(
                    self.tr("Error in loading the model.")
                )
                return

        except Exception as e:
            logger.error(f"Error in load_custom_model: {str(e)}")
            self.new_model_status.emit(
                self.tr("Error in loading custom model: Unexpected error occurred.")
            )
